  ...
  return (
    <div className="px-4 py-8">
      <div className="flex items-center justify-between mb-6 px-1">
        <h3 className="text-[10px] uppercase tracking-[0.3em] text-islamic-teal-500 font-bold">Prayer Performance</h3>
      ...
      <div className="glass-morphism rounded-3xl p-6 islamic-shadow overflow-hidden relative">
        <div className="absolute top-0 right-0 w-32 h-32 bg-islamic-teal-500/5 rounded-full blur-3xl -mr-16 -mt-16" />
          ...
          {prayers.map((prayer) => (
            <button
              ...
              className={`flex flex-col items-center justify-center min-w-[80px] aspect-square rounded-2xl transition-all duration-500 ${tracker[prayer.key]
                  ? 'bg-islamic-teal-500/20 border border-islamic-teal-500/50 text-white shadow-[0_0_15px_rgba(20,184,166,0.1)]'
                  : 'bg-islamic-deep-950/50 border border-white/5 text-gray-500 hover:border-white/10'
                }`}
            >
              <div className={`p-2 rounded-full mb-2 transition-transform duration-300 ${tracker[prayer.key] ? 'bg-islamic-teal-500 text-islamic-deep-950 scale-110 shadow-lg' : 'bg-islamic-deep-900 text-gray-600'}`}>
                {tracker[prayer.key] ? (
              ...
              <span className={`text-[10px] font-bold uppercase tracking-widest ${tracker[prayer.key] ? 'text-islamic-teal-300' : 'text-gray-600'}`}>
                {prayer.name}
              </span>
            </button>
          ))}
